...
/// release package coordinates are guarenteed to be set. This fully-qualified-ness is checked on
...
impl<'a> InstallTask<'a> {
    ...
    fn fetch_latest_pkg_ident_for(
        ...
        self.fetch_latest_pkg_ident_in_channel_for(ident, self.channel.0, token)
    }
    ...
    fn fetch_latest_pkg_ident_in_channel_for(
        ...
        let origin_package: PackageIdent = self.depot_client
            .show_package(ident, Some(channel), token)?
            .into();
