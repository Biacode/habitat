...
impl SrvClient {
    pub fn connect<T>(
        addr: &SocketAddr,
        auth_key: T,
    ) -> Box<Future<Item = SrvClient, Error = io::Error> + 'static>
    where
        T: ToString,
    {
        let auth_key = auth_key.to_string();
        let conn = TcpStream::connect(addr).and_then(move |socket| {
            ...
            client
                ...
                .and_then(move |(m, io)| {
                    m.map_or_else(
                        ...
                        move |m| {
                            // did we get an ok?
                            println!("We might want to check and see if this is all good");
                            Ok(io.into_inner())
                        },
                    )
                })
        ...
        T: protobuf::Message + protobuf::MessageStatic,
    {
        ...
        // Select & timeout here. Allow user to configure timeout on Client struct.
        let message = SrvWireMessage::new(request);
